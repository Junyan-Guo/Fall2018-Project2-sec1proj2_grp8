...
# Set WD
#setwd("C:/Users/eagle/Desktop/Group 8 Project 2/Project/Project_2_Ivy_Difference")
# Run App
#runApp(getwd())
...
shinyServer(function(input, output){
  ...
  output$tablerank2 <- renderDataTable({
    ...
    w5<-input$HappyRank
    final2dat<-data %>%
      select(Name,Earn, ADMrate,ForbesRank,AvgCost,CrimeRate,HappyRank, State, SAT)%>%
      arrange(Earn,ForbesRank)%>%
      mutate(Earn1=seq(1:nrow(data)))%>%
      arrange(AvgCost,ForbesRank)%>%
      mutate(AvgCost1=seq(1:nrow(data)))%>%
      arrange(CrimeRate,ForbesRank)%>%
      mutate(CrimeRate1=seq(1:nrow(data)))%>%
      arrange(HappyRank,ForbesRank)%>%
      mutate(HappyRank1=seq(1:nrow(data))) %>%
      mutate(new_rank=w1*Earn1+ w2*ForbesRank+w3*AvgCost1+w4*CrimeRate1+w5*HappyRank1) %>%
      arrange(new_rank)
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(data = mapStates) %>% addTiles() %>%
      ...
      addCircles(~v3()$Longitude, ~v3()$Latitude, fillColor=colorData,
                 stroke=FALSE, radius = 300000-v3()$ForbesRank*1000,
...
want on your preferred list of colleges!
  ...
  output$introduction<- renderText({"
    ...
    of finding your dream school. To get to Columbia University,  we 
  ...
  output$happy_rank <- renderPlotly({
    edu <- v3()
    cr <- edu %>% select(Name, HappyRank) %>% arrange(desc(HappyRank)) 
    cr$HappyRank <- as.numeric(as.character(cr$HappyRank))
    d <- ggplot(data = cr,aes(x=HappyRank)) + 
      geom_histogram(aes(y = ..density..), alpha = 0.7, fill = "#56B4E9") +
      geom_density(fill = "#CC79A7", alpha = 0.5) +
      theme(panel.background = element_rect(fill = '#ffffff')) + 
      ggtitle("Density of Happy Rank with Histogram Overlay")
    ggplotly(d)
  })
